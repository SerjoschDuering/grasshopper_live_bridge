...
export const ghPrompts: McpPrompt[] = [
    {
        name: 'analyze-canvas',
...
export function buildPromptMessage(promptName: string, args: Record<string, any>): string {
    const prompt = ghPrompts.find(p => p.name === promptName);
    if (!prompt) {
